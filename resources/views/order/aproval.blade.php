    ...
    <div class="content-wrapper pb-0">
          <div class="row">
              <div class="col-12">
                <div class="card">
                  <div class="card-body">
                    <h4 class="card-title">Orders Approval Need</h4>
                        ...
                        <table id="order-listing" class="table" cellspacing="0" width="100%">
                          ...
                          <tbody>
                            ...
                            <tr>
                              ...
                              @if(in_array(Auth::user()->department->name, array('Pharmacy','Admin','Riders') ))
                              <td class="text-right">
                                <form id="complete" action="{{ route('orders.update',$order->id) }}" method="post">
                                  {{ csrf_field() }}
                                  {{ method_field('PUT') }}
                                  <input type="hidden" name="status_id" value="8">
                                  <input type="hidden" name="approved" value="Yes">
                                  <button type="submit" name="submit" class="btn btn-light"><i class="mdi mdi-check text-danger"></i> Approved </button>
                                </form>
...
<script type="text/javascript">
      $("form#refund").on("submit", function(){
       ...
       $("form#complete").on("submit", function(){
        return confirm("Are you sure order Amount is Approved?");
    });
</script>
  @if(session('status'))
<script>
