    ...
    <div class="content-wrapper pb-0">
    	...
    	<div class="card">
                  <div class="card-body">
                    ...
                    <form class="form-sample" action="{{ route('orders.update',$order->id) }}" method="post" enctype="multipart/form-data">
                      {{ csrf_field() }}
                      @method('PUT')
                      <input type="hidden" name="alledit" value="0">
                      <p class="card-description"> Edit Order form </p>
                      <div class="row">
                        <div class="col-md-6">
                          <div class="form-group row">
                            <label class="col-sm-3 col-form-label">Customer Name <span>*</span></label>
                            <div class="col-sm-9">
                                <input class="form-control{{ $errors->has('customer_name') ? ' form-control-danger' : '' }}" type="text" name="customer_name" id="customer_name" placeholder="Enter Customer Name" value="{{ $order->customer_name }}" required="required" />
                         ...
                         <div class="col-md-6">
                          <div class="form-group row">
                            ...
                            <div class="col-sm-9">
                                <input class="form-control {{ $errors->has('number') ? ' form-control-danger' : '' }}" type="text" name="number" id="number" placeholder="Enter Customer Phone number" value="{{ $order->customer_number }}"  required="required" />
                      ...
                      <div class="row">
                        <div class="col-md-6">
                          <div class="form-group row">
                            ...
                            <div class="col-sm-9">
                                <input class="form-control {{ $errors->has('address') ? ' form-control-danger' : '' }}" type="text" name="address" id="address" placeholder="Enter Customer Address" value="{{ $order->customer_address }}"  required="required" />
                                @if($errors->has('address'))
                                     <label class="error mt-2 text-danger" for="$errors->has('address')">{{ $errors->first('address') }}</label>
                                  @endif
                            </div>
                          </div>
                        </div>
                      ...
                      <div class="row">
                          ...
                          <div class="form-group row">
                            ...
                            <div class="col-sm-9">
                              <select class="js-example-basic-single" name="doctor" id="doctor" style="width:100%">
                                ...
                                @php $select = $doctor->id == $order->doctor_id ? 'selected=selected' : '' @endphp
                        ...
                        <div class="col-md-6">
                          <div class="form-group row">
                            <label class="col-sm-3 col-form-label">Riders</label>
                            <div class="col-sm-9">
                              <select class="js-example-basic-single" name="rider" id="rider" style="width:100%">
                                <option value="">Select Rider..</option>
                                @foreach(App\Models\User::where('department_id',4)->get() as $user)
                                @php $select = $user->id == $order->user_id ? 'selected=selected' : '' @endphp
                                  <option {{ $select }} value="{{ $user->id }}">{{ $user->name }}</option>
                                  @endforeach
                                </select>
                                @if($errors->has('rider'))
                                     <label class="error mt-2 text-danger" for="$errors->has('rider')">{{ $errors->first('rider') }}</label>
                                  @endif
                            </div>
                          </div>
                        </div>
                      ...
                      <div class="row">
                        <div class="col-md-6">
                          <div class="form-group row">
                            <label class="col-sm-3 col-form-label">Invoice without Discount</label>
                             <div class="col-sm-9">
                                <input class="form-control {{ $errors->has('invoice_without_discount') ? ' form-control-danger' : '' }}" type="text" name="invoice_without_discount" id="invoice_without_discount" placeholder="Enter Invoice without Discount" value="{{ $order->invoice_without_discount }}" />
                                @if($errors->has('invoice_without_discount'))
                                     <label class="error mt-2 text-danger" for="$errors->has('invoice_without_discount')">{{ $errors->first('invoice_without_discount') }}</label>
                                  @endif
                            </div>
                          </div>
                        </div>
                        <div class="col-md-6">
                          <div class="form-group row">
                            <label class="col-sm-3 col-form-label">Invoice with Discount</label>
                            <div class="col-sm-9">
                                <input class="form-control {{ $errors->has('invoice_with_discount') ? ' form-control-danger' : '' }}" type="text" name="invoice_with_discount" id="invoice_with_discount" placeholder="Enter Invoice with Discount" value="{{ $order->invoice_with_discount }}" />
                                @if($errors->has('invoice_with_discount'))
                                     <label class="error mt-2 text-danger" for="$errors->has('invoice_with_discount')">{{ $errors->first('invoice_with_discount') }}</label>
                                  @endif
                            </div>
                          </div>
                        </div>
                      ...
                      <div class="row">
                        <div class="col-md-6">
                            ...
                            <div class="input-group col-sm-9">
                               <input type="text" class="form-control{{ $errors->has('invoice_file') ? ' form-control-danger' : '' }} file-upload-info" disabled placeholder="Invoice file Upload">
                              <span class="input-group-append">
                                <button class="file-upload-browse btn btn-primary" type="button">Upload</button>
                             </span>
                            </div>
                            @if($errors->has('invoice_file'))
                             <label class="error mt-2 text-danger" for="$errors->has('invoice_file')">{{ $errors->first('invoice_file') }}</label>
                            @endif
                            </div>
                        </div>
                      </div>
